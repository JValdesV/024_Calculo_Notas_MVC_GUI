...
public class EstudianteDAO {
	...
	public String registrarEstudiante(EstudianteVO miEstudiante) throws SQLException {
		...
		try {
			...
			System.out.println("No se puede registrar el estudiante verifique que el documento no exista"+e.getMessage());
			//e.printStackTrace();
			resultado = "error";
		}finally {
			preStatement = null;
	...
	public EstudianteVO consultarEstudiante(String documento) throws SQLException {
	...
	public ArrayList<EstudianteVO> consultarListaEstudiantes() throws SQLException{
		...
		String consulta = "SELECT documento_est,nombre_est,nota1,nota2,nota3,promedio FROM estudiante";
		...
		try {
			...
			System.out.println("Exepcion al traer los elementos: "+e.getMessage());
		}finally {
			result.close();
			preStatement.close();
			connection.close();
	...
	public String actualizarEstudiante(EstudianteVO estudianteVO) throws SQLException {
	...
	public String eliminarEstudiante(String documento) throws SQLException {
		...
		try {
			...
			System.out.println("Ocurrio una excepcion de SQL al momento de eliminar "+e);
			resp = "error";
		}finally {
			preStatement.close();
			connection.close();
